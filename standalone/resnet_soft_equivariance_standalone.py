#!/usr/bin/env python3
"""
Standalone script for applying soft equivariance filters to ResNet.

All softeq dependencies are inlined — only requires:
    pip install torch transformers pillow

Usage:
    python resnet_soft_equivariance_standalone.py
"""
...
def convert_cnn_to_filtered(model: nn.Module, filter_config: dict) -> nn.Module:
    ...
    def _replace_conv_in_module(parent_module, name, child_module):
        ...
        filtered_conv = FilteredConv2d(original_layer=child_module, filter=filter_module)
        setattr(parent_module, name, filtered_conv)
        filtered_count[0] += 1
        print(f"  Replaced: Conv2d({child_module.in_channels}, {child_module.out_channels}, kernel={kernel_size})")
        return True
    ...
    print(f"  soft_thresholding={soft_thresholding}")
...
def get_conv2d_layer_info(model: nn.Module) -> list:
    """Get info about all Conv2d layers in the model."""
    conv_info = []

    def _collect_conv_layers(module, prefix=""):
        for name, child in module.named_children():
            ...
            if isinstance(child, nn.Conv2d):
                ...
                _collect_conv_layers(child, full_name)

    _collect_conv_layers(model)
    return conv_info


# ============================================================
# Original resnet_soft_equivariance.py main logic
# ============================================================

def main():
    print("=" * 60)
    print("Soft Equivariance for ResNet")
    print("=" * 60)

    print("\n1. Loading pretrained ResNet model...")
    ...
    print(f"   - min_kernel_size: {filter_config['min_kernel_size']}")

    layers_before = get_conv2d_layer_info(model)
    print(f"   - Total Conv2d layers: {len(layers_before)}")

    convert_cnn_to_filtered(model, filter_config)

    layers_after = get_conv2d_layer_info(model)
    filtered_count = sum(1 for l in layers_after if l['is_filtered'])
    print(f"   - Filtered layers: {filtered_count}")
    ...
    if image is not None:
        ...
        for angle in [0, 90, 180, 270]:
            ...
            with torch.no_grad():
                resnet_outputs = model.resnet(**inputs, return_dict=True)
                if hasattr(resnet_outputs, "pooler_output") and resnet_outputs.pooler_output is not None:
                    feat = resnet_outputs.pooler_output
                else:
                    feat = resnet_outputs.last_hidden_state
                    if feat.dim() == 4:
                        feat = feat.mean(dim=[2, 3])
            features_list.append((angle, feat))
        ...
        for angle_i, feat_i in features_list:
            print(f"   {angle_i:>5}°", end="")
            for _, feat_j in features_list:
                sim = F.cosine_similarity(feat_i, feat_j).item()
