...
class GatedNonlinearity(nn.Module):
    ...
    def __init__(self,rep):
        super().__init__()
        self.rep=rep
        self.rep_size = rep.size()
        if isinstance(rep,SumRep):
            gate_idx = torch.as_tensor(gate_indices(rep),dtype=torch.long)
            self.register_buffer("gate_idx",gate_idx)
            self.is_sumrep = True
        else:
            self.is_sumrep = False
            self.is_permutation = rep.is_permutation
    def forward(self,values):
        if self.is_sumrep:
            gate_scalars = values[..., self.gate_idx]
            return gate_scalars.sigmoid() * values[..., :self.rep_size]
        if self.is_permutation:
            return values.sigmoid() * values
        gate_scalar = values[..., self.rep_size:self.rep_size+1]
        return gate_scalar.sigmoid() * values[..., :self.rep_size]

@export
class EMLPBlock(nn.Module):
    """ Basic building block of EMLP consisting of G-Linear, biLinear,
        and gated nonlinearity. """
    def __init__(self,rep_in,rep_out, only_linear=True, rpp=False, rpp_factor=0.5):
        super().__init__()
        self.linear = Linear(rep_in,gated(rep_out))
        self.rpp = rpp
        if only_linear:
            self.bilinear = nn.Identity()
        else:
            self.bilinear = BiLinear(gated(rep_out),gated(rep_out))
        self.nonlinearity = GatedNonlinearity(rep_out)
        
        if self.rpp:
            self.rpp_layer = ResLin(int(gated(rep_in).size()), int(gated(rep_out).size()), factor=rpp_factor)
        else:
            self.rpp_layer = None
    ...
    def forward(self,x):
        ...
        if self.rpp_layer is not None:
            preact = self.rpp_layer(x, preact)
...
class EMLP(nn.Module):
    ...
    def __init__(self,rep_in,rep_out,group,ch=384,num_layers=3, only_linear=True, rpp=False, rpp_factor=0.5):
        super().__init__()
        logging.info("Initing EMLP (PyTorch)")
        self.rep_in =rep_in(group)
        self.rep_out = rep_out(group)
        self.rpp = rpp
        self.rpp_factor = rpp_factor
        self.G=group
        ...
        self.network = nn.Sequential(
            *[EMLPBlock(rin,rout, only_linear=only_linear, rpp=rpp, rpp_factor=rpp_factor) for rin,rout in zip(reps,reps[1:])],
...
def MLPBlock(cin,cout):
    return nn.Sequential(nn.Linear(cin,cout),nn.ReLU())#,nn.BatchNorm0D(cout,momentum=.9),swish)#,

class ResLin(nn.Module):
    """ A linear layer with a residual connection. """
    def __init__(self,dim_in, dim_out, factor=0.1):
        super().__init__()
        self.dim_in = dim_in
    ...
    def forward(self, x, y):
        if x.shape[-1]!=self.dim_in:
            x_temp = torch.nn.functional.interpolate(x[:, None,:], size=self.dim_in, mode='linear', align_corners=True)[:,0,:]
        else:
            x_temp = x
        ...
        if y.shape[-1]!=self.dim_out:
            x_temp = torch.nn.functional.interpolate(x_temp[:, None,:], size=y.shape[-1], mode='linear', align_corners=True)[:,0,:]
        return (1-self.factor)*y + self.factor*x_temp
@export
class MLP(nn.Module):
