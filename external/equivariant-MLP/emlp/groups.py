...
try:
    ...
    # Minimal numpy fallbacks when jax-backed linear operators are unavailable.
    def I(n):
        return np.eye(n)
    ...
    def Rot90(n, k=1):
        # 2D rotation by 90 degrees on an n x n grid (flattened)
        grid = np.arange(n * n).reshape(n, n)
        rot = np.rot90(grid, k).reshape(-1)
        return np.eye(n * n)[rot]

    def SwapMatrix(n, i, j):
        mat = np.eye(n)
        mat[[i, j]] = mat[[j, i]]
        return mat
    ...
    def LazyKronsum(mats):
        # Sum of Kronecker products for Lie algebra direct sums.
        sizes = [m.shape[0] for m in mats]
        out = np.zeros((np.prod(sizes), np.prod(sizes)))
        for i, m in enumerate(mats):
            kron = np.array([[1.0]])
            for j, mj in enumerate(mats):
                kron = np.kron(kron, m if i == j else np.eye(mj.shape[0]))
            out += kron
        return out
    ...
    def vmap(fn, in_axes=0, out_axes=0):
        def mapped(*args):
            ...
            for arg, ax in zip(args, in_axes):
                if ax is None:
                    axis_args.append([arg] * len(args[0]))
                else:
                    axis_args.append(list(arg))
            results = [fn(*items) for items in zip(*axis_args)]
...
class Group(object,metaclass=Named):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.args = args
        if _HAS_JAX and isinstance(self.lie_algebra, np.ndarray):
            self.lie_algebra = jax.device_put(self.lie_algebra)
        if _HAS_JAX and isinstance(self.discrete_generators, np.ndarray):
...
def noise2sample(z, ks, lie_algebra, discrete_generators, seed=0):
    ...
    if lie_algebra.shape[0]:
        ...
        g = g @ (jax.scipy.linalg.expm(A) if _HAS_JAX else expm(A))
    M, K = ks.shape
    if M == 0:
        return g
    if _HAS_JAX:
        ...
        for k in range(K):
            ...
            for i in jax.random.permutation(pkey, M):
