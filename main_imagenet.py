...
from log_utils.wandb_utils import read_wandb_key
from models import get_model
from utils.general_utils import random_rotate_batch
...
# softeq ===
# Dataset parameters
group = parser.add_argument_group('Soft Equivariance Parameters')
parser.add_argument('--softeq_config_path', type=str, default=None, help="Path to config.yaml for softeq models.")
...
parser.add_argument('--freeze_patch_embeddings', action='store_true', default=None,
                    help='Freeze patch embedding weights (prevents gradient updates)')
parser.add_argument('--freeze_position_embeddings', action='store_true', default=None,
                    help='Freeze position embedding weights (prevents gradient updates)')
parser.add_argument('--freeze_filters', action='store_true', default=None,
...
group.add_argument('--dataset', metavar='NAME', default='torch/imagenet',
...
group.add_argument('--epochs', type=int, default=None, metavar='N',
...
group.add_argument('--rotation-augment', action='store_true', default=False,
                   help='Apply random rotation augmentation (-180 to 180 degrees) during training')
...
group.add_argument('--evaluate', action='store_true', default=False,
                   help='evaluate model on validation set only (no training)')
group.add_argument('--eval-checkpoint', default=None, type=str, metavar='PATH',
                   help='path to checkpoint for evaluation (optional, if None uses initialized model)')
...
group.add_argument('--wandb-api-path', default='./keys/wandb.txt', type=str,
                   help='Path to wandb api key file')
...
def softeq_parse_args(args):
    softeq_cfg_path = args.softeq_config_path or get_default_config_path()
    softeq_cfg_name = getattr(args, 'config_name', None)
    softeq_cfg = load_config(softeq_cfg_path, config_name=softeq_cfg_name)
    ...
    if args.num_classes:
        ...
        args.num_classes = softeq_cfg.get('model', {}).get('num_labels', 1000)
    if args.soft_thresholding is not None:
        softeq_cfg['model']['soft_thresholding'] = args.soft_thresholding
    if args.soft_thresholding_pos is not None:
        softeq_cfg['model']['soft_thresholding_pos'] = args.soft_thresholding_pos
    if args.group_type is not None:
        softeq_cfg['model']['group_type'] = args.group_type
    if args.n_rotations is not None:
        softeq_cfg['model']['n_rotations'] = args.n_rotations
    if args.freeze_patch_embeddings is not None:
        softeq_cfg['model']['freeze_patch_embeddings'] = args.freeze_patch_embeddings
    if args.freeze_position_embeddings is not None:
        softeq_cfg['model']['freeze_position_embeddings'] = args.freeze_position_embeddings
    if args.freeze_filters is not None:
    ...
    # ================= Data Configs ==================
    softeq_data_cfg = softeq_cfg.get('data', {})
    args.data_dir = softeq_data_cfg.get('root', args.data_dir)
    args.dataset = softeq_data_cfg.get('dataset', args.dataset)
    args.batch_size = softeq_data_cfg.get('batch_size', args.batch_size)
    args.workers = softeq_data_cfg.get('num_workers', args.workers)
    args.no_aug = not softeq_data_cfg.get('augmentation', not args.no_aug)
    
    # Parse augmentation parameters from config
    if 'mixup' in softeq_data_cfg:
        args.mixup = float(softeq_data_cfg['mixup'])
    if 'cutmix' in softeq_data_cfg:
        args.cutmix = float(softeq_data_cfg['cutmix'])
    if 'reprob' in softeq_data_cfg:
        args.reprob = float(softeq_data_cfg['reprob'])
    if 'label_smoothing' in softeq_data_cfg:
        args.smoothing = float(softeq_data_cfg['label_smoothing'])
    if 'auto_augment' in softeq_data_cfg:
        args.aa = softeq_data_cfg['auto_augment']

    # ================= Training Params =======================
    softeq_train_cfg = softeq_cfg.get('train', {})
    args.seed = int(softeq_cfg.get('experiment', {}).get('seed', args.seed))

    # Learning rate will be overridden if provided from command line
    if args.lr is not None:
        softeq_cfg['train']['lr'] = args.lr
    args.lr = float(softeq_train_cfg['lr'])

    # Weight decay
    args.weight_decay = float(softeq_train_cfg['weight_decay'])

    # Optimizer settings
    if 'optimizer' in softeq_train_cfg:
        args.opt = softeq_train_cfg['optimizer']
    if 'opt_eps' in softeq_train_cfg:
        args.opt_eps = float(softeq_train_cfg['opt_eps'])
    if 'opt_betas' in softeq_train_cfg:
        args.opt_betas = softeq_train_cfg['opt_betas']
    if 'momentum' in softeq_train_cfg:
        args.momentum = float(softeq_train_cfg['momentum'])

    # Learning rate schedule
    if 'scheduler' in softeq_train_cfg:
        scheduler_cfg = softeq_train_cfg['scheduler']
        scheduler_type = scheduler_cfg.get('type', 'cosine')
        print(f"Scheduler type from config: {scheduler_type}")
        if scheduler_type == 'CosineAnnealingLR':
            args.sched = 'cosine'
        
        # Parse min_lr / eta_min
        if 'eta_min' in scheduler_cfg:
            args.min_lr = float(scheduler_cfg['eta_min'])

    # Warmup settings
    if 'warmup_epochs' in softeq_train_cfg:
        args.warmup_epochs = int(softeq_train_cfg['warmup_epochs'])
    if 'warmup_lr' in softeq_train_cfg:
        args.warmup_lr = float(softeq_train_cfg['warmup_lr'])

    # Gradient clipping
    if 'clip_grad' in softeq_train_cfg:
        args.clip_grad = float(softeq_train_cfg['clip_grad'])
    
    # Layer-wise learning rate decay
    if 'layer_decay' in softeq_train_cfg:
        args.layer_decay = float(softeq_train_cfg['layer_decay'])

    # Regularization
    if 'drop' in softeq_train_cfg:
        args.drop = float(softeq_train_cfg['drop'])
    if 'drop_path' in softeq_train_cfg:
        args.drop_path = float(softeq_train_cfg['drop_path'])

    # Model EMA
    if 'model_ema' in softeq_train_cfg:
        args.model_ema = softeq_train_cfg['model_ema']
    if 'model_ema_decay' in softeq_train_cfg:
        args.model_ema_decay = float(softeq_train_cfg['model_ema_decay'])

    # Epochs
    if args.epochs is None:
        args.epochs = int(softeq_train_cfg['epochs'])
    
    # ================= Logging Params ======================
    softeq_logging_cfg = softeq_cfg.get('logging', {})
    args.log_wandb = softeq_logging_cfg.get('use_wandb', args.log_wandb)
    # args.experiment = softeq_cfg.get('experiment', {}).get('run_name', args.experiment)
    args.wandb_project = softeq_cfg.get('experiment', {}).get('project', args.wandb_project)
    args.log_interval = int(softeq_logging_cfg.get('log_interval', args.log_interval))
    args.output = softeq_logging_cfg.get('checkpoint_dir', args.output)
    # =======================================================


    return args, softeq_cfg
...
def _parse_args():
    ...
    args, softeq_cfg = softeq_parse_args(args)

    # Cache the args as a text string to save them in the output dir later
    args_text = yaml.safe_dump(args.__dict__, default_flow_style=False)
    return args, args_text, softeq_cfg


def main():
    utils.setup_default_logging()
    args, args_text, softeq_cfg = _parse_args()
    ...
    model = get_model(softeq_cfg.get('model', {}))
    ...
    if 'vit' in softeq_cfg['model']['pretrained_model'].lower():
        data_config['mean'] = (0.5, 0.5, 0.5)
        data_config['std'] = (0.5, 0.5, 0.5)
        if utils.is_primary(args):
            _logger.info(f'Using ViT normalization: mean={data_config["mean"]}, std={data_config["std"]}')
    ...
    ## dummy forward pass through the model to init weights
    _ = model(torch.randn(1, in_chans, data_config['input_size'][1], data_config['input_size'][2], device=device))
    ## dymmy forward pass end
    ...
    if args.evaluate:
        run_evaluation_only(args, model, softeq_cfg, device, model_dtype, data_config)
        return
    # ================= End evaluation-only mode =================
    ...
    if utils.is_primary(args):
        if args.experiment:
            ...
            exp_name = '-'.join([
                datetime.now().strftime("%Y%m%d-%H%M%S-%f")[:23],  # Include milliseconds
        ...
        if args.log_wandb:
            if has_wandb:
                ...
                wandb_api_key = read_wandb_key(args.wandb_api_path)
                ...
                wandb.config.update({'softeq_cfg': softeq_cfg}, allow_val_change=True)
                # Log checkpoint directory for easy reference
                wandb.run.summary["checkpoint_dir"] = output_dir
            else:
                _logger.warning(
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            ...
            model_input = random_rotate_batch(input) if args.rotation_augment else input
            
            with amp_autocast():
                output = model(model_input)
                output = output.logits if hasattr(output, "logits") else output #softeq
        ...
        if saver is not None and args.recovery_interval and (
            ...
            # Switch to eval mode before saving to ensure e2cnn filter buffers are included
            model.eval()
...
def validate(
    ...
    # Top1 and Top5 accuracy meters for rotation
    rot_top1_m = utils.AverageMeter()
    rot_top5_m = utils.AverageMeter()
    
    # Combined accuracy meters (product of regular and rotated)
    combined_top1_m = utils.AverageMeter()
    combined_top5_m = utils.AverageMeter()
    # softeq ===
    ...
    with torch.inference_mode():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            # softeq ===
            # Use already computed output and loss from above (avoid duplicate forward pass)
            running_loss += reduced_loss.item() * batch_size
            _, preds = torch.max(output, dim=1)
            correct += (preds == target).sum().item()
            total += batch_size
            
            # Augmentation evaluation using unified function
            if include_transformation and input.dim() == 4:
                # Get rotation range and group type from config
                eval_rot = softeq_cfg.get('data', {}).get('eval_rot', 90.0)  # Default to 90 degrees
                group_type = softeq_cfg.get('model', {}).get('group_type', 'rotation')  # Default to rotation
                
                # For reflection-only group, don't use rotation_config
                rotation_config = eval_rot if group_type in ['rotation', 'roto_reflection'] else None
                
                # Model wrapper to handle amp_autocast and different model types
                def model_wrapper(x):
                    # Handle different model types
                    if 'test' in model.__class__.__name__.lower() and x.dim() == 4:
                    ...
                    with amp_autocast():
                        ...
                        if hasattr(outputs, 'logits'):
                            # ViT and DINOv2 models return ImageClassifierOutput
                            return outputs.logits
                        else:
                            # TestModel returns tensor directly
                            return outputs
                
                # Use unified augmentation function with group_type
                with amp_autocast():
                ...
                rot_loss = aug_results.get('augmented_loss', loss_fn(rot_logits, target))
                rot_consistency = aug_results.get('consistency', 0.0)
                
                # Calculate top1 and top5 accuracy for rotated data
                rot_acc1, rot_acc5 = utils.accuracy(rot_logits, target, topk=(1, 5))
                
                if args.distributed:
                    rot_reduced_loss = utils.reduce_tensor(rot_loss.data, args.world_size)
                    rot_acc1 = utils.reduce_tensor(rot_acc1, args.world_size)
                    rot_acc5 = utils.reduce_tensor(rot_acc5, args.world_size)
                else:
                    rot_reduced_loss = rot_loss.data
                
                # Update rotation metrics
                rot_running_loss += rot_reduced_loss.item() * batch_size
                rot_top1_m.update(rot_acc1.item(), batch_size)
                rot_top5_m.update(rot_acc5.item(), batch_size)
                rot_total += batch_size
                
                # Update consistency metrics
                rot_consistency_sum += rot_consistency.item() * batch_size
                rot_consistency_count += batch_size
                
                # Calculate combined accuracy (product of regular and rotated)
                combined_acc1 = (acc1.item() * rot_acc1.item())**0.5 # / 100.0  # Divide by 100 since accuracies are percentages
                combined_acc5 = (acc5.item() * rot_acc5.item())**0.5 # / 100.0
                combined_top1_m.update(combined_acc1, batch_size)
                combined_top5_m.update(combined_acc5, batch_size)
                
                # Legacy accuracy calculation
                _, rot_preds = torch.max(rot_logits, dim=1)
                rot_correct += (rot_preds == target).sum().item()
            # softeq ===
            ...
            if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
                ...
                if include_transformation and rot_total > 0:
                    log_msg += (
                        f'  Rot@1: {rot_top1_m.val:>7.3f} ({rot_top1_m.avg:>7.3f})  '
                        f'Rot@5: {rot_top5_m.val:>7.3f} ({rot_top5_m.avg:>7.3f})  '
                        f'Comb@1: {combined_top1_m.val:>7.3f} ({combined_top1_m.avg:>7.3f})  '
                        f'Comb@5: {combined_top5_m.val:>7.3f} ({combined_top5_m.avg:>7.3f})'
                        f'  Consistency: {rot_consistency:.7f}'
                    )
        ...
        if include_transformation and rot_total > 0:
            ...
            print(f"Rotation Top1: {rot_top1_m.avg:.3f}%, Top5: {rot_top5_m.avg:.3f}%")
            print(f"Combined Top1: {combined_top1_m.avg:.3f}%, Top5: {combined_top5_m.avg:.3f}%")
        # softeq ===
    ...
    metrics = OrderedDict([
        ...
        ('test_rot_acc', rot_acc),
        ('test_rot_consistency', rot_consistency),
        ('rot_top1', rot_top1_m.avg if include_transformation and rot_total > 0 else 0.0),
        ('rot_top5', rot_top5_m.avg if include_transformation and rot_total > 0 else 0.0),
        ('combined_top1', combined_top1_m.avg if include_transformation and rot_total > 0 else 0.0),
        ('combined_top5', combined_top5_m.avg if include_transformation and rot_total > 0 else 0.0)
    ])

    return metrics
    # softeq ===

def run_evaluation_only(args, model, softeq_cfg, device, model_dtype, data_config):
    if utils.is_primary(args):
        _logger.info('Running in evaluation-only mode.')
    # Load checkpoint if specified
    if args.eval_checkpoint is not None:
        if utils.is_primary(args):
            _logger.info(f'Loading checkpoint from {args.eval_checkpoint}')
        load_checkpoint(model, args.eval_checkpoint, strict=True)
    else:
        if utils.is_primary(args):
            _logger.info('No checkpoint specified, evaluating initialized model.')
    # Setup AMP for evaluation
    amp_autocast = suppress
    if args.amp:
        if args.amp_impl == 'apex':
            use_amp = 'apex'
            ...
            amp_autocast = partial(torch.autocast, device_type=device.type, dtype=amp_dtype)
            if utils.is_primary(args):
                _logger.info('Using native Torch AMP for evaluation.')
    ...
    if utils.is_primary(args):
        _logger.info(
            ...
            f'  Top1: {eval_metrics["top1"]:.3f}%\n'
            f'  Top5: {eval_metrics["top5"]:.3f}%\n'
            f'  Test Loss: {eval_metrics["test_loss"]:.4f}\n'
            f'  Test Acc: {eval_metrics["test_acc"]:.4f}\n'
            f'  Rotation Loss: {eval_metrics["test_rot_loss"]:.4f}\n'
            f'  Rotation Acc: {eval_metrics["test_rot_acc"]:.4f}\n'
            f'  Rotation Top1: {eval_metrics["rot_top1"]:.3f}%\n'
            f'  Rotation Top5: {eval_metrics["rot_top5"]:.3f}%\n'
            f'  Combined Top1: {eval_metrics["combined_top1"]:.3f}%\n'
            f'  Combined Top5: {eval_metrics["combined_top5"]:.3f}%'
        )
        
        # add prefix 'eval_' to eval metrics keys for logging
        eval_metrics = {f'eval_{k}': v for k, v in eval_metrics.items()}

        # Wandb logging (mirroring per-epoch logic)
        if getattr(args, 'log_wandb', False):
            try:
                import wandb
                # If not already initialized, initialize wandb
                if not wandb.run:
                    wandb_api_key = None
                    if hasattr(args, 'wandb_api_path') and args.wandb_api_path:
                        try:
                    ...
                    wandb.init(
                        ...
                        resume="must" if getattr(args, 'wandb_resume_id', None) else None,
                        id=getattr(args, 'wandb_resume_id', None) if getattr(args, 'wandb_resume_id', None) else None,
                    )
                    wandb.config.update({'softeq_cfg': softeq_cfg}, allow_val_change=True)
                    
                wandb.log(eval_metrics)
            except ImportError:
                _logger.warning("wandb logging requested but wandb is not installed.")
