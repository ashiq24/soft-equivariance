...
from softeq.utils.group_utils import create_unit_rotation_action_matrix

class DiscreteRotationConstraints:
    """
    Discrete rotation constraints for 2D images.
    Args:
        n_rotations (int): Number of discrete rotations.
    ...
    """
    def __init__(self, n_rotations, input_size, output_size, decomposition_method='schur'):
    ...
    def create_unit_group_action_matrix(self):
        """
        Create the rotation matrix for a unit group action (input and output if specified).
        """
        self.unit_group_action_matrix = create_unit_rotation_action_matrix(self.n_rotations, self.size)
        if self.output_size is not None:
            self.unit_group_action_matrix_out = create_unit_rotation_action_matrix(
                self.n_rotations, self.output_size
            ).to(self.unit_group_action_matrix.device, dtype=self.unit_group_action_matrix.dtype)

    def apply_unit_group_action(self, image):
        """
        Apply the unit group action to the image.
        Args:
