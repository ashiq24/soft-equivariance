"""
Discrete rotation constraints for 2D and 3D vectors.
Implements rotation invariant and equivariant basis computation for vector data.
"""

import torch
import math
from softeq.utils.decompositions import schur_decomposition, svd_decomposition
from softeq.utils.group_utils_vec import (
    create_unit_rotation_action_matrix_vec,
    create_lie_algebra_action_matrix_vec,
)


class DiscreteRotationConstraintsVec:
    """
    Discrete rotation constraints for 2D and 3D vectors.
    
    Args:
        n_rotations (int): Number of discrete rotations (e.g., 4 for 0, 90, 180, 270 degrees). 
                          If -1, use continuous rotations (Lie algebra).
        ...
        axis (str): Rotation axis for 3D ('x', 'y', or 'z'), ignored for 2D.
    """
    ...
    def create_unit_group_action_matrix(self):
        """
        Create the rotation matrices for input and output.
        Sets self.unit_group_action_matrix and self.unit_group_action_matrix_out.
        """
        # Create rotation matrices for input and output
        if self.n_rotations == -1:
            # Lie algebra generators
            self.unit_group_action_matrix = create_lie_algebra_action_matrix_vec(
                self.input_size, axis=self.axis
            )
            if self.output_size != self.input_size:
                self.unit_group_action_matrix_out = create_lie_algebra_action_matrix_vec(
            ...
            self.unit_group_action_matrix = create_unit_rotation_action_matrix_vec(
                self.n_rotations, self.input_size, axis=self.axis
            )
            if self.output_size != self.input_size:
                self.unit_group_action_matrix_out = create_unit_rotation_action_matrix_vec(
                    self.n_rotations, self.output_size, axis=self.axis
                )
            else:
                self.unit_group_action_matrix_out = self.unit_group_action_matrix
    ...
    def create_unit_group_action_forward_difference_matrix(self):
        """
        Create forward difference matrix (R - I) for discrete rotations,
        or use Lie algebra generator for continuous rotations.
        """
        if self.n_rotations == -1:
            # For continuous rotations, use Lie algebra generator directly
            ...
            # For discrete rotations, compute R - I
            ...
            # normalize forward difference matrix by the discrete angle in radians of the rotations (2*pi/n_rotations)
            self.forward_difference_matrix = self.forward_difference_matrix / (2*math.pi/self.n_rotations)
            ...
            if self.output_size != self.input_size:
                ...
                # normalize forward difference matrix by the discrete angle in radians of the rotations (2*pi/n_rotations)
                self.forward_difference_matrix_out = self.forward_difference_matrix_out / (2*math.pi/self.n_rotations)
            else:
    ...
    def apply_unit_group_action(self, vectors: torch.Tensor) -> torch.Tensor:
        """
        Apply the unit group action (rotation) to vectors.
    ...
    def create_invariant_basis(self):
        ...
        corresponding to features that don't change under rotation.
        """
        with torch.no_grad():
    ...
    def create_equivariant_basis(self):
        ...
        R_out @ f(v) = f(R_in @ v) for all rotations R.
