...
Covers all four model variants:
  ...
  - filtered_dinov3       : DINOv3 for image classification
  - filtered_dinov3_seg   : DINOv3 for semantic segmentation
...
class SoftEqConfig(PretrainedConfig):
    ...
    Args:
        model_arch (str): Architecture variant. One of:
            ...
            "filtered_dinov3", "filtered_dinov3_seg".
        image_size (int, optional): Input resolution used to build RoPE coord filters for
            DINOv3. If omitted, the backbone pretrained config defaults apply.
